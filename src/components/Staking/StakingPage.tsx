...
interface Props { telegramId: string; balance: number; onStaked: (newBalance: number) => void; }
...
export default function StakingPage({ telegramId, balance, onStaked }: Props) {
  ...
  const handleStake = useCallback(async () => {
    ...
    try {
      ...
      const newBal = cur - amountNum;

      await setDoc(doc(db, 'users', telegramId, 'stakes', Date.now().toString()), {
        amount: amountNum, lockDays: selectedDays, apy: selected.apy,
        stakedAt: Date.now(), unlockAt: Date.now() + selectedDays * 86400000,
        status: 'active', projectedYield: yieldAmt,
        walletAddress: userWallet || null, serverTimestamp: serverTimestamp(),
      });
      ...
      if (userWallet && receiveWallet) {
        try {
          const tonAmt = Math.round(amountNum * 0.001 * 1e9);
          if (tonAmt >= 10000000) {
            await tonConnectUI.sendTransaction({
              validUntil: Math.floor(Date.now() / 1000) + 600,
              messages: [{ address: receiveWallet, amount: tonAmt.toString() }],
            });
            setMessage({ type:'success', text:`Staked! TON sent to staking pool. Unlocks in ${selectedDays} days.` });
          } else {
            setMessage({ type:'success', text:`BB coins staked! Unlocks in ${selectedDays} days.` });
          }
        } catch {
...
<div style={{background:'linear-gradient(145deg,#1A1A1D,#111113)',
        ...
        <p style={{color:'#5A6A79',fontSize:10,textTransform:'uppercase',letterSpacing:'0.1em',margin:'0 0 4px'}}>Available Balance</p>
      ...
      {receiveWallet && (
        <div style={{background:'rgba(79,195,247,0.06)',border:'1px solid rgba(79,195,247,0.2)',
          ...
          <p style={{color:'#4FC3F7',fontSize:10,margin:'0 0 2px',fontWeight:700}}>STAKING POOL WALLET</p>
          <p style={{color:'#7A8A99',fontSize:10,margin:0,wordBreak:'break-all',fontFamily:'monospace'}}>{receiveWallet}</p>
        </div>
      )}

      <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:8,marginBottom:16}}>
        {STAKE_PERIODS.map(period => {
          const c = COLORS[period.days]; const active = period.days === selectedDays;
          return (
            <motion.button key={period.days} whileTap={{scale:0.96}}
              onClick={() => setSelectedDays(period.days)}
              style={{background: active ? 'rgba(79,195,247,0.05)' : 'rgba(17,17,19,0.9)',
      ...
      {amountNum > 0 && (
        <motion.div initial={{opacity:0,y:8}} animate={{opacity:1,y:0}}
          ...
          <div style={{height:1,background:'#1A1A1D',margin:'4px 0 8px'}}/>
      ...
      <motion.button whileTap={isValid ? {scale:0.97} : {}} onClick={handleStake}
        disabled={!isValid || isStaking}
        style={{width:'100%',padding:'16px',borderRadius:14,fontWeight:900,fontSize:16,cursor:'pointer',
          background: isValid ? 'linear-gradient(180deg,#D0D0D0 0%,#9A9A9A 40%,#7A7A7A 60%,#B8B8B8 100%)' : 'rgba(42,42,45,0.8)',
